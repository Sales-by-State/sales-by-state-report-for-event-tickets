...
( function ( wp, config ) {
	...
	var statusOptions = ( settings.statuses || [] ).map( function ( option ) {
		return {
	...
	var API = '/sbset/v1/report';
	var DIAGNOSTICS = '/sbset/v1/diagnostics';
	var BACKFILL = '/sbset/v1/backfill';
	var DEFAULT_PER_PAGE = 25;
	var TEXTDOMAIN = 'sales-by-state-report-for-event-tickets';
	...
	function ImportStatus( props ) {
		...
		if ( ! diag.table_exists ) {
			return el(
				'div',
				{ className: 'sbset-notice is-error' },
				el( 'p', { className: 'sbset-notice__title' }, __( 'The report table is missing.', TEXTDOMAIN ) ),
		...
		return el(
			'div',
			{ className: 'sbset-importing', role: 'status', 'aria-live': 'polite' },
			el(
				'div',
				{ className: 'sbset-importing__row' },
				el( 'span', { className: 'sbset-importing__label' }, __( 'Reading past orders…', TEXTDOMAIN ) ),
				el( 'span', { className: 'sbset-importing__count' }, pct + '%' )
			),
			el(
				'div',
				{ className: 'sbset-progress' },
				el( 'div', { className: 'sbset-progress__bar', style: { width: pct + '%' } } )
			),
			el(
				'p',
				{ className: 'sbset-importing__hint' },
	...
	function readStatuses( query ) {
		var raw = query && query.sbsetStatuses;
		var defaults = ( settings.defaultStatuses || [] ).map( String );

		if ( ! raw ) {
			return defaults.length ? defaults.slice() : statusOptions.map( function ( option ) {
				return option.value;
			} );
		}
	...
	/**
	 * Compare order status IDs without number/string mismatches.
	 *
	 * @param {*} a First ID.
	 * @param {*} b Second ID.
	 * @return {boolean} Whether they are the same status.
	 */
	...
	/**
	 * Whether a status is in the current selection.
	 *
	 * @param {Array} selected Selected IDs.
	 * @param {*}     value    Candidate ID.
	 * @return {boolean} Whether it is selected.
	 */
	function isStatusSelected( selected, value ) {
		return ( selected || [] ).some( function ( s ) {
			return sameStatus( s, value );
		} );
	}
	...
	function statusLabel( value ) {
		var id = String( value );
		var match = statusOptions.filter( function ( option ) {
			return sameStatus( option.value, id );
		} )[ 0 ];
	...
	function StatusFilter( props ) {
		var selected = ( props.value || [] ).map( String );
		var label;

		if ( 1 === selected.length ) {
			label = statusLabel( selected[ 0 ] );
		...
		function toggle( value, checked ) {
			...
			next = statusOptions
				...
				.filter( function ( v ) {
					return isStatusSelected( next, v );
				} );
		...
		return el(
			'div',
			{ className: 'sbset-filter' },
			el( 'span', { className: 'sbset-filter__label' }, __( 'Order status', TEXTDOMAIN ) ),
			el( Dropdown, {
				className: 'sbset-filter__dropdown',
				contentClassName: 'sbset-status-popover',
				...
				renderToggle: function ( toggleProps ) {
					return el(
						Button,
						{
							className: 'sbset-filter__toggle',
				...
				renderContent: function () {
					return el(
						'div',
						{ className: 'sbset-status-popover__list' },
						statusOptions.map( function ( option ) {
							var checked = isStatusSelected( selected, option.value );
	...
	function Filter( props ) {
		return el(
			'label',
			{ className: 'sbset-filter' },
			el( 'span', { className: 'sbset-filter__label' }, props.label ),
			el(
				'select',
				{
					className: 'sbset-filter__select',
					value: props.value,
					onChange: function ( event ) {
	...
	function SalesByStateReport( props ) {
		...
		var country = query.sbsetCountry || settings.defaultCountry || 'US';
		var year = query.sbsetYear || settings.defaultYear;
		...
		var children = [
			...
			el(
				'div',
				{ key: 'filters', className: 'sbset-filters' },
				el( Filter, {
					label: __( 'Country', TEXTDOMAIN ),
					value: country,
					options: settings.countries || [],
					onChange: function ( value ) {
						setFilter( { sbsetCountry: value } );
					}
				} ),
				el( Filter, {
					...
					onChange: function ( value ) {
						setFilter( { sbsetYear: value } );
					}
				} ),
				el( StatusFilter, {
					value: statuses,
					onChange: function ( next ) {
						setFilter( { sbsetStatuses: next.join( ',' ) } );
					}
				} )
			)
		];
		...
		if ( data.error ) {
			children.push(
				el(
					'div',
					{ key: 'error', className: 'notice notice-error sbset-error' },
		...
		return el( 'div', { className: 'woocommerce-analytics__report sbset-report' }, children );
	}
	...
	function readUrlQuery() {
		var query = {};
		var keys = [ 'sbsetCountry', 'sbsetYear', 'sbsetStatuses', 'orderby', 'order', 'paged', 'per_page' ];
		...
		query.page = found.page || 'sbset-sales-by-state';
	...
	function mountStandalone() {
		function render() {
			var node = document.getElementById( 'sbset-root' );

			if ( ! node ) {
				return;
			}

			if ( node.getAttribute( 'data-sbset-mounted' ) ) {
				return;
			}

			node.setAttribute( 'data-sbset-mounted', '1' );
