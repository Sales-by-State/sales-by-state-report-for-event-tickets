...
class Report {
	...
	/**
	 * Fetch sales grouped by shipping state and status for one country and year.
	 *
	 * @param int    $year    Calendar year.
	 * @param string $country Two-letter country code.
	 * @return array
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT shipping_state AS state,
				 ...
				 FROM {$wpdb->prefix}sbset_order_state
	...
	private function total_by_state( array $rows, array $statuses ) {
		...
		foreach ( $rows as $row ) {
			...
			$states[ $state ]['amount'] += (float) $row['amount'];
		}
	...
	private function add_labels_and_currency( array $rows, $country ) {
		...
		foreach ( $rows as $index => $row ) {
			...
			$rows[ $index ]['state_name'] = isset( $state_names[ $code ] ) && $state_names[ $code ]
				? $state_names[ $code ]
				: ( '' === $code ? __( 'Unknown', 'sales-by-state-report-for-event-tickets' ) : $code );
	...
	private function money( $amount ) {
		...
		if ( class_exists( \TEC\Tickets\Commerce\Values\Currency_Value::class ) ) {
			$formatted = \TEC\Tickets\Commerce\Values\Currency_Value::create_from_float( $amount )->get();
